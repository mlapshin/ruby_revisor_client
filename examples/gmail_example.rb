...
class GmailTest < Test::Unit::TestCase
  ...
  def login(tab, login, password)
    ...
    tab.wait_for_condition("jquery_loaded == true", 500, 4)
    ...
    assert tab.wait_for_condition("console.log(window.location); window.location.hash == '#inbox'", 6000, 10)
  ...
  def test_mail_delivery
    ...
    sleep(1)
    assert @first_tab.wait_for_condition("jQuery('#canvas_frame').contents().find('.CoUvaf b:contains(\\'Send\\')').length == 1", 1000, 10)
    ...
    assert @second_tab.wait_for_condition("jQuery('#canvas_frame').contents().find('*:contains(\\'#{subject}\\')').length > 0", 5000, 20)
  end
end
